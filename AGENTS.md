...
- 群聊工具放在 `app/services/napcat/group_tools/`。
...
- **description 必须以主动决策视角编写，把调用决策权交给模型自身。**
  - 禁止使用"当用户要求/让你/询问…时使用"等被动句式，这会导致模型只在收到明确指令时才调用工具。
  - 应使用"当你需要…来辅助回答时，应主动调用此工具"等主动句式，引导模型在判断工具有助于提升回答质量时自行决策调用。
  - 必须列出典型触发场景，帮助模型理解何时适合调用，但场景描述应覆盖隐性需求（如"对话中提及了相关概念但你缺少上下文"），不能只覆盖显性需求（如"用户说请帮我查"）。
- 工具名使用稳定前缀 `qq__`，例如 `qq__list_group_root_files`、`qq__get_group_history_messages`。
- MCP 工具使用 `mcp__{server}__{tool}`，本地 NapCat 工具不得占用 `mcp__` 前缀。
- 信息工具只返回信息，不发送群消息。
- 引用与艾特不再作为模型可见工具暴露，统一通过 content 中的 `<Reply>` / `<At>QQ号</At>` 标记表达，由发送层转换为 NapCat 消息段。
...
- `@全体` 必须由插件配置开关控制。即使关闭，也要在通用群聊行为要求中保留 `<At>all</At>` 的合法形态，让 AI 能看到真实能力边界。
- 当 `@全体` 被关闭时，content 标记解析层应返回可恢复错误给 AI，而不是静默丢弃或伪装成普通文本。
...
- 插件负责配置开关和工具循环策略，例如：
  - 是否允许 `@全体`
  - 工具结果是否进入长期上下文
  - content 中的 `<Reply>` / `<At>` 标记如何转换为 NapCat 消息段
